...
def main():
    ...
    for group_name, group_data in data.items():
        ...
        print(
            f"  Duration: {len(group_data['transformations']) / group_data['sample_rate']:.2f} seconds"
        )
        ...
        print(
            f"    X: [{translations[:, 0].min():.2f}, {translations[:, 0].max():.2f}] {group_data['unit']}"
        )
        print(
            f"    Y: [{translations[:, 1].min():.2f}, {translations[:, 1].max():.2f}] {group_data['unit']}"
        )
        print(
            f"    Z: [{translations[:, 2].min():.2f}, {translations[:, 2].max():.2f}] {group_data['unit']}"
        )
        ...
        if "derivatives" in group_data and group_data["derivatives"]:
            ...
            if "translational_velocity" in group_data["derivatives"]:
                ...
                print(
                    f"      Magnitude range: [{vel_magnitude.min():.4f}, {vel_magnitude.max():.4f}] {group_data['unit']}/s"
                )
                print(
                    f"      Mean magnitude: {vel_magnitude.mean():.4f} {group_data['unit']}/s"
                )
            ...
            if "translational_acceleration" in group_data["derivatives"]:
                ...
                print(
                    f"      Magnitude range: [{acc_magnitude.min():.4f}, {acc_magnitude.max():.4f}] {group_data['unit']}/s²"
                )
                print(
                    f"      Mean magnitude: {acc_magnitude.mean():.4f} {group_data['unit']}/s²"
                )
            ...
            if "angular_velocity" in group_data["derivatives"]:
                ...
                print(
                    f"      Magnitude range: [{ang_vel_magnitude.min():.4f}, {ang_vel_magnitude.max():.4f}] rad/s"
                )
            ...
            if "angular_acceleration" in group_data["derivatives"]:
                ...
                print(
                    f"      Magnitude range: [{ang_acc_magnitude.min():.4f}, {ang_acc_magnitude.max():.4f}] rad/s²"
                )
            ...
            other_derivs = [
                k
                for k in group_data["derivatives"].keys()
                if k
                not in [
                    ...
                    "velocity",
                    "acceleration",  # backward compat aliases
                    "rotational_velocity",
    ...
    if len(data) > 1:
        ...
        fig_trans, axes_trans = hlp.compare_hdf5_trajectories(
            hdf5_file, group_names=group_names, component="translations"
        )

        # Compare rotations (Euler angles)
        fig_rot, axes_rot = hlp.compare_hdf5_trajectories(
            hdf5_file, group_names=group_names, component="rotations_euler"
        )

        # Compare translational velocities (if derivatives available)
        first_group_data = data[group_names[0]]
        if (
            "derivatives" in first_group_data
            and "translational_velocity" in first_group_data["derivatives"]
        ):
            fig_vel, axes_vel = hlp.compare_hdf5_trajectories(
                hdf5_file, group_names=group_names, component="translational_velocity"
            )
            print(
                "Created comparison plots for translations, rotations, and translational velocity"
            )
            ...
            if "angular_velocity" in first_group_data["derivatives"]:
                fig_ang_vel, axes_ang_vel = hlp.compare_hdf5_trajectories(
                    hdf5_file, group_names=group_names, component="angular_velocity"
                )
